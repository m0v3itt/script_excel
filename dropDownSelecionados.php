...
where tb_disponibilidade.id_dia = {$_GET['dia']} AND tb_disponibilidade.{$turno} = 1 AND 
 tb_nadadores.id_nadador in (select tb_escala.id_nadador from tb_escala where tb_escala.id_dia = {$_GET['dia']} and tb_escala.turno = '{$_GET['turno']}' and tb_escala.codigo = '{$_GET['codigo']}' 
 and tb_escala.id_praia = {$_GET['praia']} )
